		...
		internal static void Load(string keyName, object settings)
		{
			...
			foreach (var fi in t.GetFields())
			{
				...
				if (!string.IsNullOrWhiteSpace(s))
				{
					...
					{
						var parts = s.Split(',');
						if (parts.Length != 4) throw new Exception($"Field is invalid! Type: {ft}");
					...
					{
						var parts = s.Split(';');
						if (parts.Length != 2) throw new Exception($"Field is invalid! Type: {ft}");
					...
					{
						throw new Exception($"type of field is not supported! Type: {ft}");
					}
